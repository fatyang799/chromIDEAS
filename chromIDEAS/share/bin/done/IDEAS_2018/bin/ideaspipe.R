...
if (T) {
  ...
  # add by fatyang
  t=which(args == "-bin_root")
  if(length(t)>0) {
    ideaspath = args[t[length(t)] + 1]
    args = args[-c(t,t+1)]
  }
  
  # add core para by fatyang
  runideas<-function(args, out, head, inv=NULL, wait=TRUE, core=threadn, check=TRUE) {
    str=paste("ideas", paste(args, collapse=" "), "-thread", core, "-o", out)
    if(length(inv)==2) {
    ...
    #run ideas checking return value
    rt=system(paste("bash ", out, ".script", sep=""), wait=wait)
    system(paste("rm ", out, ".script", sep=""))
	if (check) {
	    if(rt!=0) {
	      stop("IDEAS quit unexpectedly")
	    }
	}
  }
  
  defheader<-function(threadn=10) { 
    head=paste("#!/bin/bash\ncd ", folder, "\nexport PATH=$PATH:",ideaspath, "/IDEAS_2018/bin\n", sep="")
    return(head)
  }
  ...
  combineState<-function(parafiles, method="ward.D", mycut=0.9, pcut=1., fout=NULL) {
    ...
    # add a test by fatyang
    if (length(parafiles_no1) == 0){
      stop("Error: there are only 1 chromatin state! please check.")
    }
    ...
    while(min(sel)==0) {
      for(i in 1:length(target)) {
        ...
        if(file.exists(paste(target[i], ".script.out", sep=""))) {
          ...
          if(max(as.integer(regexpr("abort",x)))>0) {
            sel[i]=111
          }
          if(max(as.integer(regexpr("error",x)))>0) {
            sel[i]=111
          }
          if(max(as.integer(regexpr("bad",x)))>0) {
            sel[i]=111
          }
          if(max(as.integer(regexpr("unknown",x)))>0) {
            sel[i]=111
          }
          if(max(as.integer(regexpr("cannot",x)))>0) {
            sel[i]=111
          }
        }
      }
...
if (T) {
  if(length(args) == 0) {
    message("Input arguments needed for ideaspipe.R")
    quit()
  }
  ...
  #logfile to send messages back to user
  log = NULL
  t=which(args == "-logfile")
  if(length(t)>0) {
    log = t
  }
  
  
  #default 10 threads unless changed by user input
  threadn = 10
  t=which(args == "-thread")
  if(length(t)>0) {
  ...
  #split not well tested, use at your own risk (default = NULL, add by fatyang)
  split = NULL
  t=which(args == "-split")
  if(length(t)>0) {
...
if (T) {
  ...
  if(randstart[1]>1) {
    ...
    foreach (i=1:n) %dopar% {
      ...
      runideas(targs, tout, head, tinv, core=1)
      if(type==1) {
    ...
    if(length(which(rt==111))==randstart[1]) {
      stop("IDEAS failed on all training data")
    }
    
    # splitstateflag=0
    if(splitstateflag) {
      source(paste0(ideaspath, "/splitstate.R"))
      log2value = 0
      t = which(targs == "-log2")
  ...
  #split not well tested, use at your own risk
  # split=NULL
  if(length(split) > 0) {
    tinv = as.matrix(read.table(split))
    inv = tinv[which(as.integer(tinv[,2]) + 1 < inv[2]),];  
    #put array into 3 columns and length(inv)/3 rows
    #columns: chr firstbin lastbin
    inv = array(inv, dim=c(length(inv)/3,3))
  } else {
    inv = array(c("",inv), dim=c(1, 3));  #put empty value for chrom
  }
  
  # add by fatyang
  thread_final <- ifelse(threadn>20, 10, 
                         ifelse(threadn>5, 5, threadn))
  system(paste0("rm -rf ", out, c(".cluster", ".para", ".profile", ".state")))
  
  # merge all pre-run and final run ideas
  # n=1
  target = NULL
  n = dim(inv)[1]
  if (n > 1 && threadn > 1) {
    foreach (i=1:n) %dopar% {
      target[i] = paste(out, inv[i,1], sep=".")
      runideas(args, target[i], head, inv[i,-1])
    }
    #R will wait at end of loop for all to finish
  } else {
    for(i in 1:dim(inv)[1]) {
      ...
      while (! file.exists(paste0(out, ".para"))) {
        runideas(args, target[i], head, inv[i,-1], core=thread_final, check=F)
      }
      #waits on each one to finish 
    }
  }
