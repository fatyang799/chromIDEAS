...
def s3norm(sig1_wg_raw, sig2_wg_raw, sig2_output_name, NTmethod, B_init, fdr_thresh, rank_lim, upperlim, lowerlim, p_method, common_pk_binary, common_bg_binary, cross_mark):
	...
	if True:
		sig1_raw = pd.read_csv(sig1_wg_raw, sep='\t', header=None)
		sig1_raw = np.array(sig1_raw)
		sig2_raw = pd.read_csv(sig2_wg_raw, sep='\t', header=None)
	...
	### if pk number < 0.001*total_bin_number, then use rank for reference
	if True:
	...
	### get bg and pk for sig1 and sig2
	if True:
		if cross_mark == 'T':
	...
	if True:
		if sig1_wg_raw != sig2_wg_raw:
			...
			print("The input files are identical files, please check again!. error in s3norm.py")
			exit(100)
		Ao=A
		Bo=B
	...
	if True:
		...
		sig2_norm_bedgraph = np.concatenate((sig2_raw[:,0:3], sig2_norm), axis=1)
		write2d_array(sig2_norm_bedgraph, sig2_output_name)
