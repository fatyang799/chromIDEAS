...
if true; then
	if [[ $zip == "F" ]]; then
		cat ${metadata} | awk -F "\t" -v OFS="\t" -v used_mk=${mk} '{if ($2==used_mk) print $1"."$2"."$3".ip.idsort.bedgraph"}' | sed "s#^#${input_dir}/#g" > ${mk}.file_list_tmp1
		cat ${metadata} | awk -F "\t" -v OFS="\t" -v used_mk=${mk} '{if ($2==used_mk) print $1"."$2"."$3".ctrl.idsort.bedgraph"}' | sed "s#^#${input_dir}/#g" > ${mk}.file_list_tmp2
	elif [[ $zip == "T" ]]; then
