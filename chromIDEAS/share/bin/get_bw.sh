...
# check zip or not
if true; then
	echo ${input_bedgraph} | egrep "\.gz$" 1>/dev/null 2>&1
	if [[ $? -eq 0 ]]; then
		zip=T
	fi
fi
...
if true; then
	if [[ ${zip} != "T" ]]; then
		sort -k1,1 -k2,2n ${input_bedgraph} 1>${output_bw}.sort.bedgraph 2>/dev/null
	else
		zcat ${input_bedgraph} | sort -k1,1 -k2,2n 1>${output_bw}.sort.bedgraph 2>/dev/null
	fi

	check_status "${package_name} 1 raise error, please check." "rm -rf ${output_bw}.sort.bedgraph"

	${bin_root}/bedGraphToBigWig ${output_bw}.sort.bedgraph ${genomesizes} ${output_bw} 1>/dev/null 2>/dev/null
	check_status "${package_name} 2 raise error, please check." "rm -rf ${output_bw}*"
