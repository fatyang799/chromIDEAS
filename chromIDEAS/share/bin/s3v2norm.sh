...
if true; then
	...
	# - ${cell}_${id}.${mk}.S3V2.bedgraph.info: normalization info
	...
	check_status "${package_name} 2 raise error, please check ${mk}." "rm -rf ${mk}.file_list.S3V2.txt *${mk}.S3V2.bedgraph *${mk}.S3V2.bedgraph.info.txt"
fi
...
if true; then
	...
	cat ${metadata} | awk -F '\t' -v OFS='\t' -v used_mk=${mk} '{if ($2==used_mk) print $1"_"$3"."$2".S3V2.bedgraph",$1"."$2"."$3".ctrl.idsort.bedgraph.norm.bedgraph"}' > ${mk}.getave_nbp.list.txt
	...
	check_status "${package_name} 4 raise error, please check ${mk}." "rm -rf ${mk}.file_list.S3V2.txt *${mk}.S3V2.bedgraph* ${mk}.getave_nbp.list.txt ${mk}.average_sig.bedgraph.S3V2.ave.bedgraph"
fi
