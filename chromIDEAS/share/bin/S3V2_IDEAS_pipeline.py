...
def S3V2_IDEAS_pipeline(get_sigtrack, normalization, get_bw, run_ideas, script_dir, OUTDIR, GENOME, GENOMESIZES, BLACK, metadata, bin_size, local_bg_bin, id_name, email, threads, cap_sig, IDEAS_track_link, other_parafile, overall_ref):
	...
	if normalization=='T':
		...
		mks = read1d_array(mk_list,str)
		uniq_mk_num = len(mks)
		a=call('cat '+mk_list+' | xargs -n1 -i -P '+str(threads)+' bash '+script_dir+'/get_cpk_cbg_allpk_averagesig.sh {} '+script_dir+' '+metadata, shell=True)
		...
		if uniq_mk_num==1:
			...
			if overall_ref=='F':
				a=call('cp '+mks[0]+'.average_sig.bedgraph '+mks[0]+'.average_sig.bedgraph.S3.bedgraph', shell=True)
			else:
				a=call('python3 '+script_dir+'/s3norm.py -r '+overall_ref+' -t '+mks[0]+'.average_sig.bedgraph -o '+mks[0]+'.average_sig.bedgraph.S3.bedgraph -c T', shell=True)
		else:
			add2log('multiple mks mode...', log_file)
			if overall_ref=='F':
				a=call('cat '+mk_list+' | xargs -n1 -i -P '+str(threads)+' python3 '+script_dir+'/s3norm.py -r '+script_dir+'/prior/H3K27ac.average_sig.sample200k.seed2019.bedgraph'+' -t {}.average_sig.bedgraph -o {}.average_sig.bedgraph.S3.bedgraph -c T', shell=True)
			else:
				a=call('cat '+mk_list+' | xargs -n1 -i -P '+str(threads)+' python3 '+script_dir+'/s3norm.py -r '+overall_ref+' -t {}.average_sig.bedgraph -o {}.average_sig.bedgraph.S3.bedgraph -c T', shell=True)
		add2log('S3norm average across marks......Done', log_file)

		# 3: S3V2 across samples (keep the mean value of samples with same marker are identical)
		...
		for mk in mks:
			...
			if uniq_mk_num==1:
				...
				### get input file list, format: ${cell}.${mk}.${id}.ip.idsort.bedgraph ${mk}.average_sig.bedgraph.S3.bedgraph ${cell}_${id} ${mk}
				a=call('cat '+mk+'.file_list_tmp1 | awk -F \'.\' -v OFS=\'\\t\' -v average='+mk+'.average_sig.bedgraph.S3.bedgraph \'{print $0, average, $1"_"$3, $2}\' > '+mk+'.file_list.S3V2.txt', shell=True)
				### S3V2 normalization
				b=call('bash '+script_dir+'/s3v2norm.sh -m 1_mk -n '+str(threads)+' -e '+str(local_bg_bin)+' -t '+mk+'.file_list.S3V2.txt -k '+mk+'_commonpkfdr01_z.cpk.txt -g '+mk+'_commonpkfdr01_z.cbg.txt -s '+script_dir+' -i F -l '+str(0.0001)+' -x '+log_file, shell=True)
			else:
				add2log('S3V2norm each sample multiple mks mode......', log_file)
				### get input file list, format: ${cell}.${mk}.${id}.ip.idsort.bedgraph ${mk}.average_sig.bedgraph.S3.bedgraph ${cell}_${id} ${mk}
				a=call('cat '+mk+'.file_list_tmp1 | awk -F \'.\' -v OFS=\'\\t\' -v average='+mk+'.average_sig.bedgraph.S3.bedgraph \'{print $0, average, $1"_"$3, $2}\' > '+mk+'.file_list.S3V2.txt', shell=True)
				### S3V2 normalization
				b=call('bash '+script_dir+'/s3v2norm.sh -m 2more_mk -n '+str(threads)+' -e '+str(local_bg_bin)+' -t '+mk+'.file_list.S3V2.txt -k '+mk+'_commonpkfdr01_z.cpk.txt -g '+mk+'_commonpkfdr01_z.cbg.txt -s '+script_dir+' -i '+mk+'_commonpkfdr01_z.allpk.txt'+' -l '+str(0.0001)+' -x '+log_file, shell=True)
			### get S3V2 ave
			add2log('get S3V2 ave......', log_file)
			a=call('cat '+metadata+' | awk -F \'\\t\' -v OFS=\'\\t\' -v used_mk='+mk+' \'{if ($2==used_mk) print $1"_"$3"."$2".S3V2.bedgraph"}\' > '+mk+'.tmp1.list.txt', shell=True)
			b=call('cat '+metadata+' | awk -F \'\\t\' -v OFS=\'\\t\' -v used_mk='+mk+' \'{if ($2==used_mk) print $1"."$2"."$3".ctrl.idsort.bedgraph.norm.bedgraph"}\' > '+mk+'.tmp2.list.txt', shell=True)
			c=call('paste '+mk+'.tmp1.list.txt'+' '+mk+'.tmp2.list.txt'+' > '+mk+'.getave_nbp.list.txt', shell=True)
			d=call('rm '+mk+'.tmp1.list.txt'+' '+mk+'.tmp2.list.txt', shell=True)
			e=call('Rscript '+script_dir+'/get_average_sig.R'+' '+mk+'.getave_nbp.list.txt'+' '+mk+'.average_sig.bedgraph.S3V2.ave.bedgraph', shell=True)
		...
		a=call('cat '+mk_list+' | xargs -n1 -i -P '+str(threads)+' Rscript '+script_dir+'/global_nbp_NB_cm.ok.R {}.getave_nbp.list.txt'+' {}.average_sig.bedgraph.S3V2.ave.bedgraph'+' {}_commonpkfdr01_z.cbg.txt', shell=True)
		...
		# cut 4th column
		for mk in mks:
			all_file = read2d_array(mk+'.getave_nbp.list.txt', str)
			a=call('while read -r IP CTRL; do cut -f4 $IP\'.NBP.bedgraph\' > $IP\'.NBP.txt\'; mv $IP\'.NBP.txt\' '+id_name+'_IDEAS_input_NB/; done < '+mk+'.getave_nbp.list.txt', shell=True)
			file_check = id_name+'_IDEAS_input_NB/'+all_file[0,0]+'.NBP.txt'
			### check if .NBP.txt is there
			check(file_check)
			###
			if uniq_mk_num==1:
				b=call('cut -f4 '+mk+'.average_sig.bedgraph.S3V2.ave.bedgraph.NBP.bedgraph'+' > '+mk+'.average_sig.bedgraph.S3V2.bedgraph.NBP.txt', shell=True)
				b=call('mv '+mk+'.average_sig.bedgraph.S3V2.bedgraph.NBP.txt '+id_name+'_IDEAS_input_NB/', shell=True)
	...
	if run_ideas=='T':
		...
		# add genome file into IDEAS folder
		d=call('cp '+GENOMESIZES+' '+script_dir+'/IDEAS_2018/data/'+GENOME+'.genome', shell=True)
		# add average track for input
		if uniq_mk_num==1:
			e=call('head -1 '+metadata+' | awk -F \'\\t\' -v OFS=\' \' -v S3norm_NBP_dir='+OUTDIR+'/'+id_name+'_IDEAS_input_NB/'+' \'{print "AVERAGE",$2,S3norm_NBP_dir$2".average_sig.bedgraph.S3V2.bedgraph.NBP.txt"}\' | sort -u > '+OUTDIR+'/'+id_name+'.input.tmp', shell=True)
			e=call('cat '+OUTDIR+'/'+id_name+'.input >> '+OUTDIR+'/'+id_name+'.input.tmp', shell=True)
			e=call('head -1 '+metadata+' | awk -F \'\\t\' -v OFS=\' \' -v S3norm_NBP_dir='+OUTDIR+'/'+id_name+'_IDEAS_input_NB/'+' \'{print "AVERAGE",$2"_1",S3norm_NBP_dir$2".average_sig.bedgraph.S3V2.bedgraph.NBP.txt"}\' | sort -u >> '+OUTDIR+'/'+id_name+'.input.tmp', shell=True)
			e=call('cat '+metadata+' | awk -F \'\\t\' -v OFS=\' \' -v S3norm_NBP_dir='+OUTDIR+'/'+id_name+'_IDEAS_input_NB/'+' \'{print $1"_"$3,$2"_1",S3norm_NBP_dir$1"_"$3"."$2".S3V2.bedgraph.NBP.txt"}\' | sort -u >> '+OUTDIR+'/'+id_name+'.input.tmp', shell=True)
			e=call('mv '+OUTDIR+'/'+id_name+'.input.tmp'+' '+OUTDIR+'/'+id_name+'.input', shell=True)
		### run IDEAS
		f=call('time bash '+id_name+'.sh', shell=True)
		### get cCRE list
		if uniq_mk_num==1:
			#redo as bash
